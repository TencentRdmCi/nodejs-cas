...
module.exports = function(options) {
  ...
  var pgtPathname = pgtPath(options.paths.proxyCallback);
  ...
  return function(req, res, next) {
    ...
    if (options.paths.proxyCallback) {
      ...
      params.pgtUrl = options.servicePrefix + options.paths.proxyCallback;
    } else {
      if (debug) {
...
/**
 * 拿到pgtUrl的pathname,用于接收cas回调的时候判断req.path是否等于他来接收回调
 *
 * @param options
 *  @returns {String/Boolean}
 */
function pgtPath(pgtUrl) {
  var pgtUrl = parseUrl(pgtUrl || '');
